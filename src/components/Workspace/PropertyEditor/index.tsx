...
import {
  Connector,
  ConnectorArrowTypes,
} from "@/components/Rockie/Items/src/Connector";
import {
...
const PropertyEditor: FC<PropertyEditorProps> = ({
  ...
  const [pageHeight, setPageHeight] = useState<number>(
    Consts.PAGE_HEIGHT_DEFAULT
  );
  const [pageOrientation, setPageOrientation] = useState<string>(
    Consts.PAGE_ORIENTATION_PORTRAIT
  );
  ...
  const [
    connectorDoubleLineArrowLength,
    setConnectorDoubleLineArrowLength,
  ] = useState<number>(Consts.DOUBLE_LINE_ARROW_LENGTH_DEFAULT);
  const [
    connectorDoubleLineArrowDistance,
    setConnectorDoubleLineArrowDistance,
  ] = useState<number>(Consts.DOUBLE_LINE_ARROW_DISTANCE_DEFAULT);
  ...
  const refresh = () => {
    ...
    if (currentEditor) {
      ...
      if (currentEditor.selectionLayer.getEditorItemCount() > 0) {
        ...
        setPageCustomized(
          getPageTypeName(currentEditor) == Consts.PAGE_SIZE_CUSTOM
        );
      }
    }
  };
  ...
  const getPageTypeName = (editor: Editor) => {
    ...
    PageTypes.forEach((pageType) => {
      if (
        editor.origWidth == pageType.width &&
        editor.origHeight == pageType.height
      ) {
        pageTypeName = pageType.name;
      } else if (
        editor.origWidth == pageType.height &&
        editor.origHeight == pageType.width
      ) {
        pageTypeName = pageType.name;
      }
    });
  ...
  const handleDocumentStyleChange = (
    styleName: string,
    documentThemeType: DocumentThemeType
  ) => {
    if (currentEditor) {
  ...
  const connectorSettings = (
    <div>
      ...
      >
        <Tooltip
          title={<FormattedMessage id="workspace.header.title.line-width" />}
        >
          <Select
            size="small"
            value={doubleLineGap}
  ...
  const pageSizeOptions = PageTypes.map((pageType) => {
    return {
      value: pageType.name,
      label: intl.formatMessage({ id: pageType.label }),
    };
  });
  ...
  const pageSettings = (
    <div>
      ...
      >
        ...
        <InputNumber
          min={Consts.GRID_SIZE_MIN}
          max={Consts.GRID_SIZE_MAX}
          value={gridSize}
      ...
      <div
        style={{
          display: pageSize == Consts.PAGE_SIZE_CUSTOM ? "none" : "flex",
      ...
      >
        ...
        >
          <Radio value={Consts.PAGE_ORIENTATION_PORTRAIT}>
            <FormattedMessage id="workspace.property-editor.page-setting.portrait" />
          </Radio>
          <Radio value={Consts.PAGE_ORIENTATION_LANDSCAPE}>
      ...
      <div
        style={{
          display: pageSize == Consts.PAGE_SIZE_CUSTOM ? "flex" : "none",
          justifyContent: "space-between",
          alignItems: "center",
          padding: 4,
        }}
      >
        <InputNumber
          ...
          min={Consts.PAGE_SIZE_MIN}
          max={Consts.PAGE_SIZE_MAX}
        />
  ...
  const documentThemeTypes = DocumentThemeTypes.map((documentThemeType) => (
    <div style={{ padding: "2px" }}>
      <Button
        type="default"
        onClick={() =>
          handleDocumentStyleChange(documentThemeType.name, documentThemeType)
        }
        style={{
          padding: "2px",
          display: "table",
      ...
      >
        <img
          src={`/styles/${documentThemeType.name}.png`}
      ...
      <Button
        ...
        onClick={() =>
          handleShapeStyleChange(
            documentThemeType.name,
            documentThemeType.description
          )
        }
        style={{
          padding: "2px",
  ...
  const pageItems: TabsProps["items"] = [
    {
      ...
      label: intl.formatMessage({
        id: "workspace.property-editor.page-setting.title",
      }),
    ...
    },
    {
      key: "2",
      label: intl.formatMessage({
        id: "workspace.property-editor.page-style.title",
      }),
      children: pageStyles,
    },
  ];
  ...
  const shapeItems: TabsProps["items"] = [
    {
      key: "1",
      label: intl.formatMessage({
        id: "workspace.property-editor.item-setting.title",
      }),
    ...
    },
    {
      key: "2",
      label: intl.formatMessage({
        id: "workspace.property-editor.item-style.title",
      }),
