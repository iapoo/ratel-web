import { Card, Flex, Input, message, Modal, Tooltip, Tree, TreeDataNode } from 'antd'
...
const templateMap: TemplateInfo[] = [
  {
    category: 'Basic',
    name: 'Blank Diagram',
    path: process.env.BASIC_PATH + '/template/basic/BlankDiagram.ratel',
    image: process.env.BASIC_PATH + '/template/basic/BlankDiagram.png',
  },
  {
    category: 'Basic',
    name: 'Flowchart',
    path: process.env.BASIC_PATH + '/template/basic/FlowChart.ratel',
    image: process.env.BASIC_PATH + '/template/basic/FlowChart.png',
  },
  {
    category: 'Basic',
...
const NewFileWindowPage: FC<NewFileWindowProps> = ({ visible, x, y, onWindowCancel, onWindowOk }) => {
  const intl = useIntl()
  ...
  const handleCategorySelection = (
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    e: { selected: boolean; selectedNodes: TreeDataNode[]; node: EventDataNode<TreeDataNode>; event: 'select' } | null,
  ) => {
  ...
  return (
    <div>
      <Modal
        title={<FormattedMessage id="workspace.header.window.new-file.title" />}
        centered
        open={visible}
        onOk={onOk}
        onCancel={onCancel}
