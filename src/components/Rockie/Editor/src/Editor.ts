...
export class Editor extends Painter {
  ...
  private _textEditStyleChangeListeners = new Array<(e: EditorEvent) => void>(
    0,
  );
  ...
  private _editorOperationEventListeners = new Array<
    (e: EditorOperationEvent) => void
  >(0);
  ...
  public constructor(canvasId: string | HTMLCanvasElement) {
    ...
    this._contentLayer = new ContentLayer(
      this.horizontalSpace,
      this.verticalSpace,
      this.workWidth,
      this.workHeight,
    ...
    this._moveLayer = new MaskLayer(
      this.horizontalSpace,
      this.verticalSpace,
      this.workWidth,
      this.workHeight,
    );
    this._containerLayer = new ContainerLayer(
      this.horizontalSpace,
      this.verticalSpace,
      this.workWidth,
      this.workHeight,
    ...
    this._exportLayer = new ContentLayer(
      this.horizontalSpace,
      this.verticalSpace,
      this.workWidth,
      this.workHeight,
    ...
    this._showGrid = true;
    this._rangeSelectionShape.fill.setColor(
      EditorUtils.rangeSelectionFillColor,
    );
    this._rangeSelectionShape.fill.setAlpha(
      EditorUtils.rangeSelectionFillAlpha,
    );
    this._rangeSelectionShape.stroke.setColor(
      EditorUtils.rangeSelectionStrokeColor,
    );
    this._rangeSelectionShape.stroke.setStrokeWidth(
      EditorUtils.rangeSelectionStrokeLineWidth,
    );
    this._rangeSelectionShape.stroke.setAntiAlias(
      EditorUtils.rangeSelectionStrokeAntiAlias,
    );
    this._rangeSelectionShape.stroke.setAlpha(
      EditorUtils.rangeSelectionStrokeAlpha,
    );
    this._containerSelectionShape.fill.setColor(
      EditorUtils.containerSelectionFillColor,
    );
    this._containerSelectionShape.fill.setAlpha(
      EditorUtils.containerSelectionFillAlpha,
    );
    this._containerSelectionShape.stroke.setColor(
      EditorUtils.containerSelectionStrokeColor,
    );
    this._containerSelectionShape.stroke.setStrokeWidth(
      EditorUtils.containerSelectionStrokeLineWidth,
    );
    this._selectionOutlineShape.stroke.setColor(
      EditorUtils.selectionOutlineStrokeColor,
    );
    this._selectionOutlineShape.stroke.setStrokeWidth(
      EditorUtils.selectionOutlineStrokeLineWidth,
    );
    this._selectionOutlineShape.stroke.setAntiAlias(
      EditorUtils.selectionOutlineStrokeAntiAlias,
    );
    this._selectionOutlineShape.stroke.setStrokeDashStyle(
      EditorUtils.selectionOutlineStrokeDashStyle,
    );
    this._selectionOutlineShape.filled = false;
    this._tableActiveCellShape.filled = false;
    this._tableActiveCellShape.stroke.setColor(
      EditorUtils.tableActiveCellStrokeColor,
    );
    this._tableActiveCellShape.stroke.setStrokeWidth(
      EditorUtils.tableActiveCellStrokeLineWidth,
    );
    this._tableActiveCellShape.stroke.setStrokeDashStyle(
      EditorUtils.tableActiveCellStrokeDashStyle,
    );
    this._tableActiveCellShape.stroke.setAntiAlias(
      EditorUtils.tableActiveCellStrokeAntiAlias,
    );
  ...
  public removeEditorOperationEvent(
    callback: (e: EditorOperationEvent) => void,
  ) {
    const index = this._editorOperationEventListeners.indexOf(callback);
    if (index >= 0) {
  ...
  public set action(value: Action | undefined) {
    ...
    if (this._action) {
      ...
      if (
        this._action.items[0] instanceof Connector &&
        !(this._action instanceof MyShapeAction)
      ) {
        this._action.items[0].strokeColor = SystemUtils.parseColorString(
          this._theme.connectorStrokeColor,
        )!;
        this._action.items[0].fillColor = SystemUtils.parseColorString(
          this._theme.connectorFillColor,
        )!;
        this._action.items[0].fontColor = SystemUtils.parseColorString(
          this._theme.connectorFontColor,
        )!;
      } else if (
        this._action.items[0].useTheme &&
        !(
          this._action.items[0] instanceof SvgContainer ||
          this._action.items[0] instanceof ImageContainer
        ) &&
        !(this._action instanceof MyShapeAction)
      ) {
  ...
  private updateItemTheme(item: EditorItem) {
    ...
    item.strokeColor = SystemUtils.parseColorString(
      this._theme.shapeStrokeColor,
    )!;
  ...
  public resize(origWidth: number, origHeight: number) {
    super.resize(
      origWidth * this._zoom + this.horizontalSpace * 2,
      origHeight * this._zoom + this.verticalSpace * 2,
    );
  }
  ...
  public handlePointerDown(e: PointerEvent) {
    ...
    if (this._action) {
      ...
      const isEdge = clickedEditorItem
        ? this.hasEditorItemJoint(clickedEditorItem, e.x, e.y) &&
          !this.checkParentType(clickedEditorItem, FrameEntity)
        : false;
      const inClickEditorItem = clickedEditorItem
        ? this.isInEditorItem(clickedEditorItem, e.x, e.y)
        : false;
      if (clickedEditorItem && isEdge && !inClickEditorItem) {
        //Create connector
        const targetPoint = this.findEditorItemJoint(
          clickedEditorItem,
          e.x,
          e.y,
          false,
        ...
        const startDirection = this.findConnectorDirection(
          clickedEditorItem,
          e.x,
          e.y,
        );
        ...
        const worldTargetPoint =
          clickedEditorItem.worldTransform.makePoint(targetPoint);
        ...
        if (!theSelectionLayer.hasEditorItem(clickedEditorItem)) {
          ...
          if (!clickedEditorItem.locked) {
            const targetPoint = this.findEditorItemPoint(
              clickedEditorItem,
              e.x,
              e.y,
            );
            ...
            const [targetColumn, targetColumnIndex] =
              this.isTableColumnResizable(
                clickedEditorItem,
                targetPoint.x,
                targetPoint.y,
              );
            ...
            if (targetRow) {
              ...
              const itemIndex = this.findTableItemIndex(
                clickedEditorItem,
                targetPoint.x,
                targetPoint.y,
              );
              if (this._targetItemIndex !== itemIndex) {
                ...
                this.startMoveOutline(e);
                const cellPoint = this.findEditorItemPoint(
                  this._targetItem,
                  e.x,
                  e.y,
                );
                this.updateTextCursorLocation(
                  this._targetItem,
                  cellPoint.x,
                  cellPoint.y,
                );
              ...
              if (this._target.shape.focused) {
                ...
                if (nowTime - this._targetTime < Editor.DOUBLE_CLICK_TIME) {
                  ...
                  if (this._target instanceof Connector) {
                    let origItemInfo = OperationHelper.saveEditorItem(
                      this._target,
                    );
                    this.createTextBoxInConnector(
                      this._target,
                      targetPoint.x,
                      targetPoint.y,
                    );
                    let editorItemInfo = OperationHelper.saveEditorItem(
                      this._target,
                    );
                    let operation = new Operation(
                      this,
  ...
  private initializeTextArea() {
    ...
    this._textArea.addEventListener('input', (e: any): void => {
      ...
      if (this._target && this._targetItem && e.data) {
        const [origEditorItemInfo, startIndex, endIndex] =
          this.beginShapeTextEditOperation(this._target);
        this._targetItem.shape.insert(e.data);
        this.finishShapeTextEditOperation(
          origEditorItemInfo,
          startIndex,
          endIndex,
        );
  ...
  private updateTextCursorLocation(
  ...
  private findEditorItemDetail(
    ...
    if (editorItem instanceof Connector && excludeConnector) {
      ...
      if (
        shape.intersects(
          x - Editor.TEST_RADIUS,
          y - Editor.TEST_RADIUS,
          Editor.TEST_SIZE,
          Editor.TEST_SIZE,
        )
      ) {
      ...
      if (!(editorItem instanceof TableEntity)) {
        for (let i = count - 1; i >= 0; i--) {
          ...
          let childResult = this.findEditorItemDetail(
            child,
            x,
            y,
            excludeConnector,
  ...
  public findEditorItemDetailById(
    editorItem: EditorItem,
    id: string,
  ): EditorItem | undefined {
    let result;
  ...
  public findEditorItemJoint(
    editorItem: EditorItem,
    x: number,
    y: number,
    inEditorItem: boolean,
  ): Point2 {
    const shape = editorItem.shape;
    const centerX = x;
    const centerY = y;
    ...
    targetX = this.alignToGridSize(
      (targetX - this.horizontalSpace) / this._zoom,
    );
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  private isTableColumnResizable(
    tableEntity: TableEntity,
    x: number,
    y: number,
  ): [boolean, number] {
  ...
  private handlePointMoveinAction(e: PointerEvent, action: Action) {
    ...
    let ex = this.alignToGridSize(
      (e.x - this.horizontalSpace) / this._zoom - width / 2,
    );
    let ey = this.alignToGridSize(
      (e.y - this.verticalSpace) / this._zoom - height / 2,
    );
    ...
    action.items.forEach((item) => {
      const checkResult =
        containerEntity &&
        this_.checkIfCreationInContainer(item, containerEntity);
      if (!checkResult) {
  ...
  private handlePointMoveInCreatingConnector(e: PointerEvent) {
    ...
    const isEdge = editorItem
      ? this.hasEditorItemJoint(editorItem, e.x, e.y) &&
        !this.checkParentType(editorItem, FrameEntity)
      : false;
    ...
    if (editorItem && isEdge) {
      ...
      connector.end = new Point2(
        (e.x - this.horizontalSpace) / this._zoom,
        (e.y - this.verticalSpace) / this._zoom,
      );
  ...
  private handleTargetRowResizing(e: PointerEvent) {
    ...
    const newHeight =
      tableEntity.items[rowIndex * columnCount].height +
      e.y / this._zoom -
      this._startPointY / this._zoom;
    const nextNewTop =
      tableEntity.items[(rowIndex + 1) * columnCount].top +
      e.y / this._zoom -
      this._startPointY / this._zoom;
    const nextNewHeight =
      tableEntity.items[(rowIndex + 1) * columnCount].height -
      e.y / this._zoom +
      this._startPointY / this._zoom;
    if (newHeight >= minHeight && newHeight <= maxHeight) {
      for (let i = 0; i < columnCount; i++) {
        const shapeEntity = tableEntity.items[
          rowIndex * columnCount + i
        ] as ShapeEntity;
        shapeEntity.boundary = Rectangle.makeLTWH(
          shapeEntity.left,
          shapeEntity.top,
          shapeEntity.width,
          newHeight,
        );
        const nextShapeEntity = tableEntity.items[
          (rowIndex + 1) * columnCount + i
        ] as ShapeEntity;
  ...
  private handleTargetColumnResizing(e: PointerEvent) {
    ...
    const newWidth =
      tableEntity.items[columnIndex].width +
      e.x / this._zoom -
      this._startPointX / this._zoom;
    const nextNewLeft =
      tableEntity.items[columnIndex + 1].left +
      e.x / this._zoom -
      this._startPointX / this._zoom;
    const nextNewWidth =
      tableEntity.items[columnIndex + 1].width -
      e.x / this._zoom +
      this._startPointX / this._zoom;
    if (newWidth >= minWidth && newWidth <= maxWidth) {
      for (let i = 0; i < rowCount; i++) {
        const shapeEntity = tableEntity.items[
          columnIndex + rowCount * i
        ] as ShapeEntity;
        shapeEntity.boundary = Rectangle.makeLTWH(
          shapeEntity.left,
          shapeEntity.top,
  ...
  private handleAddEditorItemIntoEditorItem(
    editorItemInfo: EditorItemInfo,
    parentId: string,
  ) {
  ...
  private handleShapeTextEdit(operation: Operation, isUndo: boolean) {
    ...
    if (editorItem) {
      const newEditorItem = this.updateEditorItemDetail(
        editorItem as Item,
        editorItemInfo,
      );
      this.selectionLayer.removeAllEditorItems();
  ...
  private handleRangeSelecting(e: PointerEvent) {
    ...
    this._rangeSelectionShape.boundary = Rectangle.makeLTWH(
      left,
      top,
      width,
      height,
    ...
    if (this._inContainerSelection && containerEntity) {
      ...
      for (let i = 0; i < selectionCount; i++) {
        ...
        if (selection.parent === containerEntity) {
          ...
          const rotation =
            selection.rotation.radius - containerEntity.rotation.radius;

          selection.boundary = Rectangle.makeLTWH(
            left,
            top,
            selection.width,
            selection.height,
  ...
  public getSelectionBoundary(): [number, number, number, number] {
    ...
    for (let i = 0; i < selectionCount; i++) {
      ...
      const rightTopPoint = worldTransform.makePoint(
        new Point2(selection.width, 0),
      );
      const rightBottomPoint = worldTransform.makePoint(
        new Point2(selection.width, selection.height),
      );
      const leftBottomPoint = worldTransform.makePoint(
        new Point2(0, selection.height),
      );
  ...
  private checkIfSelectionInContainer(
  ...
  public checkIfCreationInContainer(
    editorItem: EditorItem,
    containerEntity: ContainerEntity,
  ): boolean {
  ...
  private handleDefaultPointMove(e: PointerEvent) {
    ...
    const theSelectionLayer: SelectionLayer = this
      .selectionLayer as SelectionLayer;
    const editorItem = this.findEditorItem(e.x, e.y, false);
    ...
    if (editorItem && isEdge) {
      ...
      if (!theSelectionLayer.hasEditorItem(editorItem)) {
        ...
        if (editorItem instanceof TableEntity) {
          if (!editorItem.locked) {
            ...
            const [targetRow] = this.isTableRowtResizable(
              editorItem,
              targetPoint.x,
              targetPoint.y,
            );
            const [targetColumn] = this.isTableColumnResizable(
              editorItem,
              targetPoint.x,
              targetPoint.y,
            );
      ...
      if (theHoverLayer.getEditorItemCount() > 0) {
        ...
        if (hoverEditorItem instanceof Connector) {
          ...
          if (
            hoverEditorItem.left - Holder.PADDING <
              (e.x - this.horizontalSpace) / this._zoom &&
            hoverEditorItem.right + Holder.PADDING >
              (e.x - this.horizontalSpace) / this._zoom &&
            hoverEditorItem.top - Holder.PADDING <
              (e.y - this.verticalSpace) / this._zoom &&
            hoverEditorItem.bottom + Holder.PADDING >
              (e.y - this.verticalSpace) / this._zoom
          ) {
  ...
  private handleCreationAction(e: PointerEvent) {
    if (this._action) {
      ...
      if (
        ...
        let [left, top, right, bottom] = Editor.getItemsBoundary(
          this._action.items,
        );
        let width = right - left;
        let height = bottom - top;
        ...
        this.selectionLayer.removeAllEditorItems();
        this.selectionLayer.addEditorItems(this._action.items);
        let editorItemInfos = OperationHelper.saveEditorItems(
          this._action.items,
        );
        let operation = new Operation(
          this,
      ...
      for (let j = 0; j < sourceConnectorCount; j++) {
        ...
        if (connector.source === editorItem) {
          ...
          if (connectorInSelection) {
            ...
            connector.start = new Point2(
              connector.start.x + alignMoveX,
              connector.start.y + alignMoveY,
            );
      ...
      for (let j = 0; j < targetConnectorCount; j++) {
        ...
        if (connector.target === editorItem) {
          ...
          if (connectorInSelection) {
            connector.autoRefreshOrthogonalPoints = false;
            connector.end = new Point2(
              connector.end.x + alignMoveX,
              connector.end.y + alignMoveY,
            );
            connector.autoRefreshOrthogonalPoints = true;
          } else {
            connector.end = new Point2(
  ...
  private handleMoveOutline(e: PointerEvent) {
    ...
    if (
      this.selectionLayer.getEditorItemCount() === 1 &&
      this.selectionLayer.getEditorItem(0) instanceof Connector
    ) {
      return;
    }
    const [left, top, right, bottom] = this.getSelectionBoundary();
    this._selectionOutlineShape.boundary = Rectangle.makeLTWH(
      left,
      top,
      right - left,
      bottom - top,
    ...
    if (requireRemove) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (selection.parent) {
          ...
          const rotation =
            selection.rotation.radius + selection.parent.rotation.radius;
          selection.boundary = Rectangle.makeLTWH(
            left,
            top,
            selection.width,
  ...
  public findConnectorDirection(
    ...
    if (matrix) {
      ...
      if (
        Math.abs(position.y - shape.height / 2) <
        Math.abs(position.x - shape.width / 2)
      ) {
  ...
  private beginShapeTextEditOperation(
    editorItem: EditorItem,
  ): [EditorItemInfo, number, number] {
    if (this._target && this._targetItem) {
      ...
      return [
        editorItemInfo,
        editorItem.shape.startIndex,
        editorItem.shape.endIndex,
      ];
    }
  }
  ...
  private finishShapeTextEditOperation(
    origEditorItemInfo: EditorItemInfo,
    startIndex: number,
    endIndex: number,
  ) {
    if (this._target && this._targetItem) {
  ...
  private static getItemsBoundary(items: Item[]) {
    ...
    for (let i = 0; i < itemCount; i++) {
      ...
      const rightBottomPoint = worldTransform.makePoint(
        new Point2(item.width, item.height),
      );
      const leftBottomPoint = worldTransform.makePoint(
        new Point2(0, item.height),
      );
      const itemLeft = Math.min(
        leftTopPoint.x,
        rightTopPoint.x,
        rightBottomPoint.x,
        leftBottomPoint.x,
