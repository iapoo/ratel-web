...
import { Matrix, Rectangle,  RoundRectangle,  Shape, } from "@/components/Engine";
...
export class SvgUtils {
    ...
    private static parseAttrs(attrs: any, shape: EntityShape) {
        if(attrs) {
            if(attrs['fill']) {
                const color = SystemUtils.parseColorString(attrs[`fill`])
                if(color) {
                    shape.fill.setColor(color)
                }
            }
            if(attrs['stroke']) {
                const color = SystemUtils.parseColorString(attrs[`stroke`])
                if(color) {
                    shape.stroke.setColor(color)
                }
            }
            if(attrs['stroke-width']) {
                const strokeWidth = parseFloat(attrs[`stroke-width`])
                shape.stroke.setStrokeWidth(strokeWidth)
            }
            if(attrs['fill-opacity']) {
                const opacity = parseFloat(attrs[`fill-opacity`])
                shape.fill.setAlpha(opacity)
            }
    ...
    private static parsePointArray(pointArray: PointArray, svgShape: SvgShape) {
        pointArray.forEach((point, index, array) => {
            if(index == 0) {
    ...
    private static parseCommon(element: Element, shape: EntityShape, svgShape: SvgShape) {
        ...
        SvgUtils.parseAttrs(attrs, shape)
    }
    ...
    private static parsePathCommand(path: Path, shape: SvgShape) {
        ...
        pathArray.forEach((pathCommand, index, array) => {
